...
describe('AuthService', () => {
  ...
  describe('signUp', () => {
    ...
    it('should handle registration errors', async () => {
      ...
      const result = await authService.signUp('test@example.com', 'password123', 'Test', 'User');
      expect(result).toEqual({ user: null, error: { message: 'Email already exists' } });
    });
  });
  ...
  describe('getUser', () => {
    it('should return user data when authenticated', async () => {
      ...
      // Import the setter method from api to properly set module scope var
      const { setAuthToken } = await import('../lib/api');
    ...
    it('should return null when not authenticated (fetch fails)', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: false,
  ...
  describe('getSession', () => {
    it('should return user wrapper when authenticated', async () => {
      const mockUser = {
        id: '123',
        email: 'test@example.com',
        firstName: 'Test',
    ...
    it('should return null when no user returned', async () => {
      mockFetch.mockResolvedValueOnce({
        ok: true,
